...
import { selectIsAuthHydrated } from "@/features/auth/auth.slice";
...
export default function Topbar() {
  ...
  const handleLogout = () => {
    dispatch(logout());
    document.cookie = "accessToken=; Path=/; Max-Age=0; SameSite=Lax";  router.refresh();
    router.push("/login");
  };

  const isHydrated = useAppSelector((s) =>
    selectIsAuthHydrated({ auth: s.auth })
  );
  // const token = useAppSelector((s) => s.auth.token);

  // Early return to avoid flicker before we know auth state
  if (!isHydrated) {
    return (
      <header className="sticky top-0 z-50 bg-white/95 border-b border-emerald-100 shadow-sm">
        ...
        <div className="flex items-center gap-4 py-4">
          ...
          <div className="hidden sm:flex items-center gap-2 ml-2">
            ...
            {token ? (
              ...
              <Link
                href="/login"
                className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white font-medium rounded-xl hover:bg-emerald-700 transition-all"
              >
              ...
              <div className="pt-2 mt-2 border-t border-emerald-100 space-y-2">
                ...
                {token ? (
                  ...
                  <Link
                    href="/login"
                    onClick={() => setMobileMenuOpen(false)}
                    className="w-full flex items-center gap-3 px-4 py-3 bg-emerald-600 text-white font-medium rounded-xl hover:bg-emerald-700 transition-all"
                  >
